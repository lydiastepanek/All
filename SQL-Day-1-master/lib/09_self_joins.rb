...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
  SELECT DISTINCT
    first.num, first.company, stops.name, second.num, second.company
  FROM
  (SELECT
  b.stop_id stop_idb1, b.num, b.company
  FROM
    routes b
  LEFT OUTER JOIN
    routes a ON (a.company = b.company AND a.num = b.num)
  WHERE
    a.stop_id = 53
    ) first
  JOIN
  (SELECT
  b.stop_id stop_idb2, b.num, b.company
  FROM
    routes b
  LEFT OUTER JOIN
  ...
  ON
    first.stop_idb1 = second.stop_idb2
  JOIN
    stops on (stops.id = first.stop_idb1)
  SQL
end
